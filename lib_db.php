...
function connect(){
	...
	mysqli_select_db($link,'btl') or die('Could not select database.');//mysqli_select_db thay đổi db sau ket noi mac dinh
...
function exec_select($sql){
	...
	$res = $link->query($sql);//thực thi truy vấn và gán
	$row = array();
	...
	if ($res ){
		...
		while( $row = $res->fetch_array(MYSQLI_ASSOC) )//tra ve kết quả câu truy vấn dạng mảng
		{				
			$ret[]= $row ;
		}
		//mysql_free_result($res);
		$res->free_result();//giải phóng bôj nhớ
	}	
	close();
	return $ret;
}
...
function sql_str($val){
	...
	//return $val;
	return str_replace("'" ,"\\'", $val);//thay thế ký tự
